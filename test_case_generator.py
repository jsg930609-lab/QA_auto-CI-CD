...
2. **사전조건 포함**: 필요한 페이지 접속을 Test step 첫 부분에 추가
3. **구체적 단계**: URL, CSS 선택자, 입력값을 구체적으로 명시
4. **검증 명확화**: Expected Result를 검증 가능하게 작성
5. **URL 정확성**: 검증 조건이 실제 URL 패턴과 일치하는지 확인
6. **입력값 검증 제거**: 입력 필드에 값을 넣은 후 확인하는 단계는 불필요하므로 제거
7. **연속 액션**: 검색어 입력 후 Enter는 대기 없이 바로 실행

# 특별 지침
- **Google 검색창**: textarea[name='q'] 사용
- **검색 실행 패턴**: 반드시 "1) 페이지 접속 → 2) 검색어 입력 → 3) Enter → 4) URL 확인" 순서
- **URL 검증**: 
  * Google 메인: 'google.com' 확인
  * 이미지 검색: 'imghp' 확인 (NOT 'images')
  * 검색 결과: 'search?q=' 확인
  * 뉴스: 'news.google.com' 확인
- **입력값 검증 금지**: "검색창에 'test'가 입력되었는지 확인" 같은 단계는 제거
...
3. 검색창에 'Playwright'가 입력되었는지 확인  ← 제거 필요
4. Enter 키 입력
```
...
2. 검색창(textarea[name='q'])에 'Playwright' 입력
3. Enter 키 입력
4. URL에 'search?q=Playwright' 포함 확인
```
...
[
  {{
    ...
    "title": "원본 제목 유지",
    "precondition": "원본 유지",
    "test_step": "보정된 단계 (입력값 검증 제거, 독립적으로)",
...
응답:"""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "당신은 QA 테스트 전문가입니다. 테스트케이스를 독립적이고 완전하게 만들며, 불필요한 입력값 검증은 제거합니다."
                    },
                    {
                        "role": "user",
                        "content": prompt
